...
class UIConsola:
    ...
    def ejecutar_turno_de_la_casa(self):
        ...
        if self.jugador_gano():
            ...
            fichas_ganadas = self.apuesta_actual
        elif self.casa_gano():
        ...
        self.jugador.agregar_fichas(fichas_ganadas)
        print(f"Has ganado o perdido {abs(fichas_ganadas)} fichas.")
        ...
        if not self.jugador.tiene_fichas():
            print("Te has quedado sin fichas. El juego ha terminado.")
            self.salir()
    ...
    @staticmethod
    def mostrar_manos(self, mano_casa, mano_jugador, destapar_casa=False):
        print(f"\n{'MANO DE LA CASA':<15}\n{str(mano_casa):<15}")
